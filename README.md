...
`list_conditions` and `describe_condition` are enriched with the ProSuite QA Quick Reference, fetched on first use: each test gains the readable description and the test family it belongs to, and searches match that wording rather than only the API docstring. If it cannot be fetched, both tools work as before without it.
