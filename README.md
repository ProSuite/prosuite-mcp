...
| `PROSUITE_MCP_STATE_DIR`           | per-user state | Async run database, results, and local QA output |
| `PROSUITE_MCP_MAX_CONCURRENT_RUNS` | `1`            | Maximum background verifications                 |
...
With a spec loaded, the assistant calls `describe_spec` to see which specifications and workspaces it defines, then `start_xml_verification` to queue one against your data. The start call immediately returns a `run_id`; `get_verification_status` reports elapsed time, latest progress and output locations, and `get_verification_result` returns the final error summary. The spec goes to the ProSuite service as-is, so per-condition filters and defaults are applied exactly as your domain experts configured them. `search_spec` browses the conditions in a spec by keyword.
...
Without a spec, the assistant uses `list_conditions` and `describe_condition` to find and configure conditions from scratch, then `start_verification` to queue them ad-hoc. Datasets take a `filter_expression` here too, but only one per dataset per run, so a spec that filters the same feature class differently in two conditions cannot be reproduced this way. The older `run_verification` and `run_xml_verification` tools remain available for compatibility, but keep the MCP request open and should only be used for deliberately short runs.
